...
\newcommand{\cit}[1]{\mintinline{js}{#1}}
...
As you can see, \cit{RequestResponseEvent} is a node of the async tree that represents request listener, but it's not a simple argument or async object. \cit{RequestResponseEvent} implements \cit{Event} interface and it needs to be treated in a special way, so it requires more flexibility of the whole system. But we can create \cit{Event} via \cit{AsyncObject} (it'll described in section 9).
...
\cit{TreeNode}, \cit{SimpleTreeNode}, \cit{AsyncTreeNode}, \cit{NotDefinedAsyncTreeNode}, \cit{Event}, \cit{As}.
...
Either of methods \cit{asyncCall} or \cit{syncCall} must be defined(implemented) in extended classes. If both of them are implemented then only \cit{asyncCall} will be invoked. \cit{asyncCall} is used for async calls, so it must return function of the async call. \cit{syncCall} is used for synchronous processing, so it must return blocking function. The entire logic of using these methods will be shown in the \cit{AsyncTreeNode} class. 
...
Methods \cit{isAsyncObject} and \cit{isEvent} check if argument extends \cit{AsyncObject} and \cit{Event} respectively. Both of them use method \cit{classChain}
...
First of all, we extends \cit{ReadBufferByFD} from \cit{AsyncObject}. Constructor has the same parameters as corresponding async call has except callback, we don't need to pass it there. Every of these parameters might be or not an \cit{AsyncObject}. 
...
In the method \cit{onResult} (that handles the same result that callback of the async call provides) we return \cit{buffer}. So, it means that this async object represents buffer.
...
Here we must use explicit definition of the sync call in the \cit{syncCall} method, because we need to return buffer. And we don't need to a callback because it's a synchronous operation. Here we don't need to override \cit{onResult} method, because we override here the sync call that returns what we need.

\subsection{Why don't use SyncObject abstraction for the sync calls?}

Two reasons. First of them is technical: it's just easier to implement \cit{AsyncObject} that can wrap either sync or async call. 
...
As you can see \cit{body} use the same arguments as the event of the \cit{readStream}. 
...
The main problem of \cit{Event} is that it cannot encapsulate async objects, because it's being replaced by corresponding function(which is actually \\ \cit{body}) in a moment of construction of the async tree that contains this event. So, if you want use an event that can be constructed by async objects you can simply create \cit{AsyncObject} that represents a function that is the \cit{body} of the event:
