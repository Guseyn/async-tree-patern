...
The main problem of \cit{Event} is that it cannot encapsulate async objects, because it's being replaced by corresponding function(which is actually \\ \cit{definedBody}) in a moment of construction of the async tree that contains this event. So, if you want use an event that can be constructed by async objects you can simply create \cit{AsyncObject} that represents a function that is the \cit{definedBody} of the event:
...
class OpenEvent extends AsyncObject {
  ...
  definedSyncObject() {
    return (...resultsFromAsyncObjects) => {
        // This is definedBody of the event
        return (fd) => {
